...
export default function RoleManagementPage() {
  ...
  return (
    <AppShell
      currentPath="/roles"
      pageTitle="Role-Based Access Control (RBAC) Roles"
      pageSubtitle="Manage system default and custom organizational roles and permission assignments."
      breadcrumbItems={[{ label: 'Identity' }, { label: 'Role Management' }]}
      actionButtons={
        <button
          onClick={() => alert('Create Custom Role Dialog')}
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-xs font-semibold text-white rounded-lg shadow-lg transition-colors"
        >
          + Create Custom Role
        </button>
      }
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {roles.map((role) => (
          <div
            key={role.name}
            className="bg-[#172033] border border-white/10 rounded-xl p-5 shadow-lg space-y-3"
          >
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/5 text-xs text-slate-400">
              ...
              <button
                onClick={() => alert(`Editing permissions for role: ${role.name}`)}
