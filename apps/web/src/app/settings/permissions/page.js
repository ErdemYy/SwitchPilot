...
exports.default = PermissionRegistryPage;
const react_1 = __importDefault(require("react"));
const AppShell_1 = require("../../layouts/AppShell");
function PermissionRegistryPage() {
    ...
    return (<AppShell_1.AppShell currentPath="/permissions" pageTitle="Permission Registry Explorer" pageSubtitle="Granular permission codes mapped across Organization, Identity, Hardware, and Configuration domains." breadcrumbItems={[{ label: 'Identity' }, { label: 'Permissions' }]}>
      <div className="space-y-6">
        {permissionGroups.map((pg) => (<div key={pg.group} className="bg-[#172033] border border-white/10 rounded-xl p-5 shadow-lg space-y-3">
            <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider text-blue-400">
              {pg.group} Domain
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {pg.perms.map((p) => (<div key={p.code} className="p-3 bg-[#111827] border border-white/5 rounded-lg space-y-1">
