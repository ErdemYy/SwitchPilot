...
export default function TopologyViewerPage() {
  const [activeLayer, setActiveLayer] = useState<string>('LAYER_2');
  const [layoutMode, setLayoutMode] = useState<string>('FORCE_DIRECTED');
  ...
  return (
    <AppShell
      ...
      pageTitle="Enterprise Network Topology & Discovery"
      pageSubtitle="Automated seed-based network discovery, multi-layer topology mapping (L1/L2/L3), dependency analysis, and Single Point of Failure (SPOF) detection."
      breadcrumbItems={[{ label: 'Platform' }, { label: 'Topology Mapper' }]}
      actionButtons={
        <div className="flex items-center space-x-2">
          <Button variant="secondary" size="sm" onClick={() => alert('Exporting SVG/JSON Graph...')}>
            📥 Export Graph
          </Button>
          <Button variant="primary" size="sm" onClick={handleStartDiscovery} disabled={isScanning}>
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <Card title="Topology Layers & Layout Controls">
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Topology Layer View:</label>
                <div className="grid grid-cols-3 gap-1.5">
                  {['LAYER_1', 'LAYER_2', 'LAYER_3'].map((l) => (
                    <button
                      key={l}
                      onClick={() => setActiveLayer(l)}
                      className={`px-2.5 py-1.5 rounded text-[10px] font-bold ${
                        activeLayer === l ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                      }`}
                    >
                      {l.replace('_', ' ')}
                    </button>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {['FORCE_DIRECTED', 'HIERARCHICAL', 'RADIAL', 'GRID'].map((mode) => (
                    <button
                      key={mode}
                      onClick={() => setLayoutMode(mode)}
                      className={`px-2 py-1 rounded text-[10px] font-semibold ${
                        layoutMode === mode ? 'bg-blue-950 text-blue-300 border border-blue-800' : 'bg-slate-900 text-slate-400'
                      }`}
                    >
                      {mode.replace('_', ' ')}
                    </button>
          ...
          <Card title="Single Point of Failure (SPOF) Analysis">
            <div className="space-y-2.5 text-xs">
              <div className="p-3 bg-red-950/40 border border-red-800 rounded-lg space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="danger" size="sm">CRITICAL SPOF</Badge>
                </div>
              ...
              <div className="p-3 bg-amber-950/40 border border-amber-800 rounded-lg space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="warning" size="sm">HIGH SPOF</Badge>
                </div>
                <p className="text-[10px] text-slate-300">
        ...
        <div className="lg:col-span-8 space-y-4">
          <Card title={`Interactive Topology Graph Canvas (${activeLayer} - ${layoutMode})`}>
            <div className="space-y-3">
              {/* Filter bar */}
              <div className="flex items-center justify-between">
                <input
                  type="text"
                  placeholder="Filter nodes by hostname, IP, role..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="bg-[#111827] border border-white/10 rounded px-3 py-1.5 text-xs text-slate-200 w-64 focus:outline-none focus:border-blue-500"
                />
                ...
                <div className="mt-6 pt-4 border-t border-white/10 space-y-2 relative z-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                    {topologyLinks.map((link, idx) => (
                      <div key={idx} className="p-2 bg-[#111827] rounded border border-white/5 space-y-0.5">
