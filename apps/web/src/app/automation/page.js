...
exports.default = AutomationStudioPage;
const react_1 = __importStar(require("react"));
const AppShell_1 = require("../../layouts/AppShell");
const ui_1 = require("@switchpilot/ui");
function AutomationStudioPage() {
    const [selectedTemplate, setSelectedTemplate] = (0, react_1.useState)('CREATE_VLAN');
    const [isCanaryEnabled, setIsCanaryEnabled] = (0, react_1.useState)(true);
    const [cronExpression, setCronExpression] = (0, react_1.useState)('0 2 * * *');
    const templates = [
    ...
    return (<AppShell_1.AppShell currentPath="/automation" pageTitle="Enterprise Automation Studio" pageSubtitle="Reusable templates, visual DAG workflow builder, Canary rollouts, cron scheduling, and multi-channel notifications." breadcrumbItems={[{ label: 'Platform' }, { label: 'Automation Engine' }]} actionButtons={<div className="flex items-center space-x-2">
          <ui_1.Button variant="secondary" size="sm" onClick={() => alert('Template Exported to Marketplace')}>
            📦 Export Template
          </ui_1.Button>
          <ui_1.Button variant="primary" size="sm" onClick={() => alert('Trigger Workflow Engine')}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <ui_1.Card title="Reusable Automation Template Library (15 Templates)">
            <div className="space-y-2.5 pt-1">
              {templates.map((tpl) => (<div key={tpl.code} onClick={() => setSelectedTemplate(tpl.code)} className={`p-3 rounded-lg border text-xs cursor-pointer transition-all ${selectedTemplate === tpl.code
          ...
          <ui_1.Card title="Canary Deployment & Maintenance Window Rules">
            <div className="space-y-3 text-xs">
              <div className="flex items-center justify-between p-3 bg-[#111827] rounded border border-white/5">
                ...
                <input type="checkbox" checked={isCanaryEnabled} onChange={() => setIsCanaryEnabled(!isCanaryEnabled)} className="rounded text-blue-600 focus:ring-0 w-4 h-4"/>
              </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <ui_1.Card title="Visual Drag & Drop Workflow Builder Canvas">
            <div className="space-y-3 pt-1">
              ...
              <div className="space-y-2">
                {workflowNodes.map((node, i) => (<div key={node.id} className="p-3 bg-slate-950 rounded-lg border border-white/10 flex items-center justify-between text-xs font-mono">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-sans font-semibold ${node.status === 'COMPLETED' || node.status === 'APPROVED'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Card title="Cron Job Scheduler">
              <div className="space-y-2 text-xs">
                <label className="block text-slate-400">Cron Schedule Expression:</label>
                <input type="text" value={cronExpression} onChange={(e) => setCronExpression(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded p-2 text-slate-200 font-mono"/>
                <span className="text-[10px] text-slate-500 block">Next Run: Tomorrow at 02:00:00 UTC</span>
              </div>
            </ui_1.Card>

            <ui_1.Card title="Multi-Channel Notifications">
