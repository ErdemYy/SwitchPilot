...
import { Search, Dropdown } from '@switchpilot/ui';
...
export const Topbar: React.FC<TopbarProps> = ({ onOpenCommandPalette }) => {
  ...
  const userMenuItems = [
    ...
    { label: 'Sign Out', onClick: () => alert('Signed out'), variant: 'danger' as const },
  ];

  return (
    <header className="h-16 bg-[#111827] border-b border-white/10 px-6 flex items-center justify-between sticky top-0 z-30 shadow-md">
      ...
      <div className="w-80">
        <Search
          ...
          placeholder="Global Search (Devices, IPs, Tasks)..."
        />
      </div>
      ...
      <div className="flex items-center space-x-3">
        ...
        <Dropdown
          trigger={
            <button className="px-3 py-1.5 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors flex items-center space-x-1">
        ...
        <Dropdown
          trigger={
            <button className="flex items-center space-x-2 p-1 rounded-lg hover:bg-slate-800 transition-colors">
