...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [workspace, setWorkspace] = useState('Global Production NOC');
  ...
  return (
    <aside
      className={`fixed left-0 top-0 bottom-0 z-40 bg-[#111827] border-r border-white/10 flex flex-col transition-all duration-300 ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* Brand Header */}
      ...
      {!collapsed && (
        <div className="p-3 border-b border-white/5">
          ...
          <select
            ...
            className="w-full bg-[#172033] border border-white/10 rounded-md px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-600"
          >
