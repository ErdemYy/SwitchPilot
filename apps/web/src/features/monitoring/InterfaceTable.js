...
function InterfaceTable() {
    ...
    return (<ui_1.Card title="Top Interface Utilization & Error Counters">
      <div className="overflow-x-auto">
        <table className="w-full text-xs text-left">
          ...
          <tbody className="text-slate-300 font-mono">
            {topInterfaces.map((row, i) => (<tr key={i} className="border-b border-white/5 hover:bg-white/[0.02]">
                <td className="py-2 pr-4">{row.device}</td>
                <td className="py-2 pr-4">{row.iface}</td>
                <td className="py-2 pr-4">
                  <div className="flex items-center space-x-2">
                    <div className="w-16 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                      <div className={`h-full rounded-full ${row.util > 85 ? 'bg-red-500' : row.util > 60 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${row.util}%` }}/>
                    </div>
                ...
                <td className="py-2">
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${row.status === 'UP'
