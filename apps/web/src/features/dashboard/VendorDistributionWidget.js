...
exports.VendorDistributionWidget = void 0;
const react_1 = __importDefault(require("react"));
const VendorDistributionWidget = () => {
    const vendors = [
    ...
    return (<div className="bg-[#172033] border border-white/10 rounded-xl p-5 shadow-lg space-y-4">
      ...
      <div className="w-full bg-slate-900 rounded-full h-3 flex overflow-hidden p-0.5 border border-white/5">
        {vendors.map((v) => (<div key={v.name} className={`h-full ${v.color} first:rounded-l-full last:rounded-r-full transition-all duration-300`} style={{ width: `${v.percent}%` }} title={`${v.name}: ${v.count} devices (${v.percent}%)`}/>))}
      </div>

      {/* Legend List */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
        {vendors.map((v) => (<div key={v.name} className="flex items-center space-x-2 text-xs">
...
exports.VendorDistributionWidget = VendorDistributionWidget;
