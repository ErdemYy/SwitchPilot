...
exports.DeviceImportModal = void 0;
const react_1 = __importStar(require("react"));
const DeviceImportModal = ({ isOpen, onClose, onImportSuccess, }) => {
      ...
      <div className="relative w-full max-w-2xl bg-[#111827] border border-white/10 rounded-xl shadow-2xl overflow-hidden p-6 space-y-5">
        ...
        {step === 'upload' ? (<div className="space-y-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3 text-xs">
                <label className="flex items-center space-x-1 cursor-pointer">
                  <input type="radio" name="format" checked={format === 'csv'} onChange={() => setFormat('csv')} className="text-blue-600 focus:ring-0"/>
                  <span>CSV File (.csv)</span>
                </label>
                <label className="flex items-center space-x-1 cursor-pointer">
                  <input type="radio" name="format" checked={format === 'json'} onChange={() => setFormat('json')} className="text-blue-600 focus:ring-0"/>
            ...
            <div className="flex justify-end space-x-2 pt-2">
              ...
              <button onClick={() => setStep('preview')} className="px-4 py-2 text-xs bg-blue-600 text-white font-semibold rounded hover:bg-blue-700">
                Continue to Preview →
              </button>
            </div>
            ...
            <div className="max-h-60 overflow-y-auto border border-white/10 rounded-lg">
              <table className="w-full text-left text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-white/5">
                  {mockPreviewData.map((item, idx) => (<tr key={idx} className="hover:bg-slate-800/40">
                      <td className="p-2 font-bold">{item.hostname}</td>
                      <td className="p-2 font-mono">{item.ip}</td>
                      <td className="p-2">{item.vendor}</td>
                      <td className="p-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] ${item.status.startsWith('VALID')
...
exports.DeviceImportModal = DeviceImportModal;
