...
import { StatusChip, StatusType } from '@switchpilot/ui';
...
export interface DeviceCardProps {
  ...
  onSelect?: (device: DeviceCardData) => void;
  onAction?: (action: string, device: DeviceCardData) => void;
}

export const DeviceCard: React.FC<DeviceCardProps> = ({ device, onSelect, onAction }) => {
  const vendorBadgeColors: Record<string, string> = {
  ...
  const badgeClass =
    vendorBadgeColors[device.vendor.toUpperCase()] ||
    'bg-slate-800 text-slate-300 border-slate-700';

  return (
    <div
      onClick={() => onSelect && onSelect(device)}
      className="bg-[#172033] border border-white/10 hover:border-blue-600/50 rounded-xl p-4 shadow-lg transition-all duration-200 space-y-3 cursor-pointer group"
    >
      {/* Header Row */}
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <span
            className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wider ${badgeClass}`}
          >
            {device.vendor}
          </span>
